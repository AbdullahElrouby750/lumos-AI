...
import cv2
import threading #! rouby: too much threading, system will crash out irl. Need to implement a more robust task queue and management system. or we may use the threading system already in rouby's project. test if it'll fit here.
import time
import os
import re
import platform
...
def listen_and_respond(frame_snapshot):
    ...
    with sr.Microphone() as source:#! rouby: replace with flutter voice input handling, this is just for testing on desktop
        try:
...
def main():
    ...
    if not cap.isOpened():
        print(">>> [ERROR] Cannot open camera.") #! rouby: each error print like this shoulb be spoken too.
        return
